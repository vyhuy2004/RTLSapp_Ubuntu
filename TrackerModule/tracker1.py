...
class LocationTracker():
    def __init__(self, tk_frame = None):
        self.debug = False
    ...
    def visualize(self):
        ...
        if not (self.tk_frame is None):
            self.bar = FigureCanvasTkAgg(self.fig, self.tk_frame)
            self.bar.draw()
            self.bar.get_tk_widget().grid(row = 0, column = 0, sticky = 'WE')
        else:
            plt.show()
        self.ani = animation.FuncAnimation(self.fig, self._update, interval=250, 
